...
<table id='<?=$dataSource["divid"]?>_form' class='formTable' align=center width=<?=$width?> >
	...
	<?php
		$dtNames=array();
		if(strlen($dataSource['datatable_colnames'])>0) {
			$dtNames=explode(",",$dataSource['datatable_colnames']);
		}
		$dtCols=array_flip($dtCols);
		foreach($dtCols as $a=>$b) {
			if(isset($dtNames[$b])) $dtCols[$a]=$dtNames[$b];
			else {
				$x=str_replace("_"," ",$a);
				$dtCols[$a]=ucwords($x);
			}
		...
		foreach($dtCols as $a=>$b) {
			$sf="<tr><td class='columnName'>$b</td><td class='columnEqual'>:</td><td class='columnInput'>
				<input id='$a' name='$a' class='textfield' type='text' value='' />
				</td></tr>";
			echo $sf;
		}
